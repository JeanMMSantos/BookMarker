...
public class FreeUserResource {
	...
	public ResponseEntity<Void> delete(@PathVariable Long id) {
		freeUserService.delete(id);
		return ResponseEntity.noContent().build();
	}

    @PutMapping(value = "/{id}")
    public ResponseEntity<FreeUser> updated(@PathVariable Long id, @RequestBody FreeUser freeUser) {
        freeUser = freeUserService.update(id, freeUser);
        return ResponseEntity.ok(freeUser);
    }
}
